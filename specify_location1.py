...
# フォント登録
# ===============================================================
LabelBase.register(name="NotoSans", fn_regular="NotoSansJP-Regular.ttf")
# ===============================================================
# Android 権限
# ===============================================================
try:
...
class RoundedButton(Button):
    def __init__(self, text="", font_size=28, size=(180,80),
                 bg_color=(0.671,0.906,0.510,1), **kwargs):
        super().__init__(**kwargs)
        ...
        self.font_name = "NotoSans"  # ← 修正ポイント！
        self.color = (0,0,0,1)

        with self.canvas.before:
            self._bg_color_inst = Color(rgba=bg_color)
...
class MainScreen(FloatLayout):
    ...
    def _build_back_button(self):
        btn = RoundedButton(text="", size=(100,100), bg_color=(0.671,0.906,0.510,1))
        btn.pos_hint = {'x':0.02, 'top':0.98}

        img = Image(source="img/back.png", size_hint=(None,None),
                    size=(60,60), pos_hint={'center_x':0.5,'center_y':0.5})

        btn.add_widget(img)
        btn.bind(on_press=lambda inst: print("戻るボタン押された"))
        self.add_widget(btn)

    def _build_bottom_panel(self):
        panel_height = 300
        bottom_panel = FloatLayout(
            size_hint=(1, None),
            height=panel_height,
            pos_hint={'x': 0, 'y': 0}
        )
        ...
        with bottom_panel.canvas.before:
            Color(0.925, 0.957, 0.91, 0.95)
            bottom_panel.bg = RoundedRectangle(
                pos=bottom_panel.pos,
                size=bottom_panel.size,
                radius=[25]
            )

        bottom_panel.bind(
            pos=lambda *a: setattr(bottom_panel.bg, 'pos', bottom_panel.pos),
            size=lambda *a: setattr(bottom_panel.bg, 'size', bottom_panel.size)
        )
        ...
        btn1 = RoundedButton(text="指定する", size=(180,80))
        btn2 = RoundedButton(text="共有する", size=(180,80))
        button_layout.add_widget(btn1)
        button_layout.add_widget(btn2)
        bottom_panel.add_widget(button_layout)
